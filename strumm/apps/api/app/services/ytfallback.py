...
def ytdlp_playlist(url: str, *, limit: int = 200) -> list[dict[str, Any]]:
    """
    Extract the track list of a YouTube / YouTube Music playlist using yt-dlp's
    extractor (metadata only, no audio downloads).

    Browser-only import policy: the API server's egress IP is YouTube-blocked,
    so ``ytmusicapi``/youtubei.js cannot be used to fetch a playlist from this
    host. yt-dlp goes through a different network path and is the *allowed*
    server-side fallback (see ``app/routes/playlist.py``).

    ``yt-dlp`` must be installed. Returns raw importer-shaped items (same shape
    as ``search_fallback``). Empty on any failure so the caller falls through
    gracefully. Unlike a search, a playlist fetch returns the concrete tracks.
    """
    if not _ytdlp_available():
        return []

    # Guessing a playlist id back out of the url triggers a fetch anyway; hand
    # it straight to yt-dlp which resolves playlist=ids on any YouTube URL.
    try:
        import yt_dlp
        ...
        logger.warning(
            f"yt-dlp playlist fetch failed for url={url!r}: "
            f"{type(exc).__name__}: {exc!s:.120}"
        )
        return []

    entries = info.get("entries") or []
    # A single-video result (no entries) may still be a valid playlist edge
    # case; treat it as a one-track list.
    ...
    for e in entries:
        ...
        channel = (
            ...
            or e.get("uploader")
            or e.get("channel")
            or (", ".join(a.get("name", "") for a in e.get("artists", []) if a.get("name")))
            or ""
        )
        album = ""
        if e.get("album"):
            album = e.get("album")
        elif e.get("album_artist"):
            album = e.get("album_artist")
        duration_seconds = int(e.get("duration") or 0)
        thumbs = e.get("thumbnails") or []
        thumb_url = ""
        if thumbs:
